...
export class Paginator extends React.Component<IPaginatorProps> {
					...
					<button
						onClick={() =>
							this.props.onChangePage?.(
								Math.min(Math.ceil(this.props.rowCount / this.props.rowsPerPage), this.props.page + 1),
							)
						}
	...
	public render = () => {
		const rangeWithDots: any = this.pagination(
			this.props.page,
			Math.ceil(this.props.rowCount / this.props.rowsPerPage),
		);
		const x: number = (this.props.page - 1) * this.props.rowsPerPage + 1;
		const y: number = Math.min(
			this.props.rowCount,
			(this.props.page - 1) * this.props.rowsPerPage + this.props.rowsPerPage,
		);

		return (
			<>
				<div className='grid sm:grid-cols-9 px-4 py-3 text-xs font-semibold tracking-wide text-base-400 uppercase bg-base-800 border-t border-base-700'>
